            ...
            if (rawString.front() == '^') {
                ...
                if (rawString.front() == "(" && endBracketPos > 0) {
                    ...
                    if (fontDefinition->name != "Maestro") {
                        endString.append(u'<font face="' + String::fromStdString(fontDefinition->name) + '"/>');
                    }
                ...
                size_t endBracketPos = rawString.find(")");
                if (rawString.front() == "(" && endBracketPos > 0) {
                    endString.append(u'<font size="');
                    endString.append(String::fromStdString(rawString.substr(1, endBracketPos - 1) + u'"/>');
                ...
                endString.append(Char::fromAscii('\u00A9'));
...
void FinaleParser::importTexts()
{
    ...
    for (std::shared_ptr<texts::FileInfoText> fileInfoText : fileInfoTexts) {
        ...
        std::string fileInfoValue = fileInfoTextBlock ? fileInfoTextBlock->getText(/*trimTags*/ true, util::EnigmaString::AccidentalStyle::Unicode) ? std::string();
