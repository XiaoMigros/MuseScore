...
public:
    ...
    Q_INVOKABLE DirectionV stemDirection(int pitch) { return drumset()->stemDirection(pitch); }

    /// The name (untranslated) of the drumset note for a given pitch.
    /// \param pitch The pitch of the note to find the name for.
    Q_INVOKABLE String name(int pitch) { return drumset()->name(pitch); }
    ...
    /// - \p tremoloType - tremolo type of this variant, one of
