...
void SlurTieLayout::slurPos(Slur* item, SlurTiePos* sp, LayoutContext& ctx)
{
    ...
    const SlurSegment* bs = !item->segmentsEmpty() ? item->frontSegment() : nullptr;
    ...
    if (staffHasStems) {
        ...
        if (scr->up() == ecr->up() && scr->up() == startUp) {
    ...
    if (sa1 == SlurAnchor::NONE || sa2 == SlurAnchor::NONE) {   // need stemPos if sa2 == SlurAnchor::NONE
        ...
        if (stem1 || trem) {     //sc not null
            Beam* beam1 = sc->beam();
            if (beam1 && (beam1->elements().back() != sc) && (sc->up() == startUp)) {
